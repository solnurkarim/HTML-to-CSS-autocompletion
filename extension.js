...
function registerItemProvider(languageFilter) {
  const completionProvider = languages.registerCompletionItemProvider(
    languageFilter,
    {
      provideCompletionItems: (document, end, token, context) => {
        // check if position is not in property or pseudo-class range
        const start = new Position(end.line, 0);
        const range = new Range(start, end);
        const lineText = document.getText(range);
        let match;
        for (let i = lineText.length - 1; i > 0; i--) {
          if (lineText[i] === ":") {
            const diff = lineText.lastIndexOf(" ") - lineText.lastIndexOf(":");
            if (diff > 1) match = " ";
            else match = ":";
            break;
          }
...
const configInputMethods = {
  ...
  "Restore configurations to default": async function() {
    ...
    registerProviders();
    window.showInformationMessage(
      "HTML to CSS autocompletion: Configurations have been restored to default."
    );
